...
- **LLM 是大脑，代码是工具**：代码提供工具、安全边界、状态、trace、资源限制和编排，不要把开放性策略全部硬化到代码分支里。
- **少加非安全限制**：除安全、资源、权限和数据一致性外，不要用代码限制 LLM 是否可以尝试、修正或继续调用工具。
- **检查用于修正，不用于默认阻断**：结构化解析、规则校验、视觉检查和质量评分优先产出 warning / Observation，交回 LLM 修正；不要轻易让它们成为流程终止条件。
- **先可见，再优化**：开放任务先保证 best effort 结果可见、失败可复盘，再迭代准确性；不要为了追求一次性正确而让用户什么都看不到。
- **调度保持可塑性**：能由 LLM 根据上下文决定的计划、工具选择和修正步骤，优先通过 Prompt 和工具契约表达；代码只控制循环上限和安全边界。
