...
**问题**：几何辅助线绘图调试中，代码层曾通过 Prompt 禁止 `python:figure`、结构化抽取 / 校验失败就不画图、前端在生成回复期间禁用输入等方式增加限制。这些限制没有提升智能体效果，反而让流程在小问题上中断，用户看不到可用结果。

**根因**：把开放性 AI Agent 当成传统确定性流程来写，试图用代码枚举辅助线、证明策略、调度分支和用户交互状态。对于辅助线绘图这类开放任务，规则无法穷举，过早硬化会让 LLM 失去修正和尝试空间。

**规则**：
1. Agent 流程的默认目标是让结果可见、可复盘、可修正，不是让代码在前置检查中提前拒绝尝试
2. 代码层只保留必要的安全边界、资源限制、工具契约、trace 和重试上限；不要添加与安全无关的硬阻断
3. 校验、结构化解析、视觉检查和规则匹配应优先产出 warning / Observation，交回 LLM 修正，而不是默认终止流程
4. 对开放任务，优先让 LLM 生成计划、调用工具、观察结果并修正；代码不要替 LLM 穷举策略
5. 交互层不要因为后置流程仍在运行就阻断用户继续提问；文字答案结束后应允许继续输入，后置结果归属原消息
6. 新增限制前必须回答：它是在保护安全 / 资源，还是在替 LLM 做判断；后者默认不加，除非已有真实失败证据和回退路径
