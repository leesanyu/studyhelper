...
- [ ] 持久化用户消息、AI 消息、Dify `conversation_id/message_id`、知识点标签和题目上下文摘要。（当前完成用户/AI 消息、Dify `conversation_id/message_id`；知识点标签和题目上下文摘要待补）
...
- [ ] 实现会话列表接口，按更新时间倒序返回标题、最后消息、学科和知识点摘要。（当前完成基础列表和匿名用户过滤）
- [ ] 实现会话详情与消息历史接口，返回消息、附件、知识点、Dify 映射信息和沙箱图片引用。（当前完成基础详情和附件 ID）
- [ ] 实现会话状态更新：新题目覆盖上下文，继续追问复用上下文，直接解答后保留上下文。
- [ ] 明确删除策略：Sprint 2 只做软删除或不做删除，避免误删调试数据。
- [ ] 编写服务层测试，验证消息顺序、会话状态和匿名用户隔离。（当前为 API 级内存服务基础测试）
...
| Story | 工作量 | 状态 |
|-------|--------|------|
| Story 2.1: 初始化 FastAPI 工程 | 1 天 | 🚧 部分完成 |
| Story 2.2: 设计并创建 PostgreSQL 数据库表 | 1 天 | ✅ 已完成 |
| Story 2.3: 对接 Dify API，封装流式聊天接口 | 1.5 天 | 🚧 部分完成 |
| Story 2.4: 实现图片上传代理 | 1 天 | 🚧 部分完成 |
| Story 2.5: 对话管理 | 1 天 | 🚧 部分完成 |
